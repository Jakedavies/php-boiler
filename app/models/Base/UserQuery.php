 ...
 *
 * @method     ChildUserQuery leftJoinCharity($relationAlias = null) Adds a LEFT JOIN clause to the query using the Charity relation
 * @method     ChildUserQuery rightJoinCharity($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Charity relation
 * @method     ChildUserQuery innerJoinCharity($relationAlias = null) Adds a INNER JOIN clause to the query using the Charity relation
 *
 * @method     \CharityQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'defaultdb', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, email, password, type FROM user WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query by a related \Charity object
     *
     * @param \Charity|ObjectCollection $charity the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
    ...
    public function filterByCharity($charity, $comparison = null)
    {
        if ($charity instanceof \Charity) {
            return $this
                ->addUsingAlias(UserTableMap::COL_ID, $charity->getOwner(), $comparison);
        } elseif ($charity instanceof ObjectCollection) {
            return $this
                ->useCharityQuery()
                ->filterByPrimaryKeys($charity->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByCharity() only accepts arguments of type \Charity or Collection');
        }
    }
     ...
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function joinCharity($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return \CharityQuery A secondary query class using the current class as primary query
     */
    public function useCharityQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinCharity($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Charity', '\CharityQuery');
    }
